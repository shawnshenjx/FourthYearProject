...
while ischar(tline)
    ...
    data.kbPos = cols(1:3);
    data.kbRot = cols(4:7);
    data.hmdPos = cols(8:10);
    data.hmdRot = cols(11:14);
    data.mPos = cols(15:17);
    data.t = cols(end);
...
ax = gca;
view(-150, 25)
axis([-0.5 0.5 0. 1.0 0.5 1.5]*1000)
xlabel('x-axis')
ylabel('y-axis')
zlabel('z-axis')
...
kbFrameH = plotFrame([],tData(1).kbPos,tData(1).kbRot);
...
for i = 1:5:N
    ...
    kbFrameH = plotFrame(kbFrameH,data.kbPos,data.kbRot);
...
%       This is the Tait-Bryan Sequence of z-yí-x? (intrinsic rotations) where the intrinsic rotations are known as: yaw, pitch and roll
