...
data = struct;
data.kbPos=[-0.2683,-0.0570,0.7959];
data.kbRot=[0.86449740,0.15243420,0.47168920,-0.08317154];
tline = fgetl(fid);
tline = fgetl(fid);
...
for i = 1:1:N2
    ...
    hmdFrameH = plotFrame3L(hmdFrameH,hmdFrameHH,data.hmdRot,data.hmdRot);
    drawnow
